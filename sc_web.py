...
which allows the user to see and edit stats / matchup data for Tech 1 land units from the game."""
import sqlite3
...
@app.route("/")
def home():
    ''' home page code is contained within this function'''
...
@app.route("/boundary")
def boundary():
    ''' info page on boundaries is contained iwthin this function'''
...
@app.route("/counters/<int:id3>")
def unit_counter(id3):
...
def sort_units(stat):
    ...
    try:
        with sqlite3.connect(TARGET_DATABASE_FILE) as connection:
            ...
            column_names = [
                "Units_T1_Land.Health",
                "Units_T1_Land.DPS",
                "Units_T1_Land.Mass_Cost",
                "Units_T1_Land.Energy_Cost",
                "Units_T1_Land.Range",
                "Units_T1_Land.Speed",
...
def add_units():
    ...
    return render_template(
        "add_units.html", counters=data.counters, units=results, response=data.response
    )


@app.route("/add_units", methods=["POST"])
def add_units_submitted():
    '''contains code regarding the state of the form when user submits new unit data'''
        ...
        with sqlite3.connect(TARGET_DATABASE_FILE) as connection:
            ...
            for counter in range(data.counters):
                ...
                cursor.execute(
                    ...
                    (
                        new_id,
                        response[f"counter_name{counter}"],
                        response[f"counter_description{counter}"],
                    ),
                )
...
def delete_units_submitted():
    '''this function includes info regarding the form state once a unit has been deleted'''
    with sqlite3.connect(TARGET_DATABASE_FILE) as connection:
        cursor = connection.cursor()
        response = request.form
